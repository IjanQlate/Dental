 ...
 function appStatus($status){
  ...
  return $statuses[$status];
   ...
   $query = mysqli_query($db,"SELECT *,a.app_ID as appointment_id,b.bill_ID as bill_id FROM appointment as a LEFT JOIN bill as b ON b.app_ID=a.app_ID LEFT JOIN patient as p ON a.user_ID=p.user_ID  left join treatment as t ON t.treatment_ID = a.treatment_ID WHERE status = 3 ");
   ...
   <!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Patient's Appointment</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            ...
            ?>
                  <tr>
                    ...
                    <td style="color: green;"><?php echo appStatus($data['status'])?></td>
                      <td>
                            <?php if(is_null($data['bill_id'])){ ?>
                            <a href="create.php?id=<?= $data['appointment_id'] ?>"><input type="button" class="btn badge-success" name="view" value="Create Invoice"></a>
                            
                            <?php }else{ ?>
                            <a href="view.php?id=<?= $data['bill_id'] ?>"><input type="button" class="btn badge-info" name="view" value="View Invoice"></a>
                            <?php } ?>
